...
public class BundleTaskExecutor extends TaskExecutor {
  ...
  public BundleTaskExecutor(
      BundleRepository bundleRepository, ArchivedBundleRepository archivedBundleRepository) {
    super();
  ...
  public void execute() {
    ...
    List<ArchivedBundle> archivedBundles =
        bundles.parallelStream()
            .map(b -> modelMapper.map(b, ArchivedBundle.class))
