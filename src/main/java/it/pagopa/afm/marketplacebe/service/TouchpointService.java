...
public class TouchpointService {
  ...
  @Autowired private TouchpointRepository touchpointRepository;
  ...
  public Touchpoints getTouchpoints() {
    ...
    PageInfo pageInfo = PageInfo.builder().itemsFound(touchpointList.size()).totalPages(1).build();

    return Touchpoints.builder().touchpointList(touchpointList).pageInfo(pageInfo).build();
  }
  ...
  public Touchpoint createTouchpoint(TouchpointRequest request) {
    ...
    it.pagopa.afm.marketplacebe.entity.Touchpoint entry =
        it.pagopa.afm.marketplacebe.entity.Touchpoint.builder()
            .createdDate(now)
  ...
  public void deleteTouchpoint(String idTouchpoint) {
    ...
    if (!bundles.isEmpty()) {
      throw new AppException(
          AppError.TOUCHPOINT_BAD_REQUEST,
          String.format("%s is used in at least one bundle.", idTouchpoint));
    }
  ...
  private it.pagopa.afm.marketplacebe.entity.Touchpoint getTouchpointById(String idTouchpoint) {
    Optional<it.pagopa.afm.marketplacebe.entity.Touchpoint> touchpoint =
        touchpointRepository.findById(idTouchpoint);
