...
public class CosmosRepository {
  ...
  public List<Bundle> getBundlesByNameAndType(
    ...
    return IterableUtils.toList(
        cosmosTemplate.runQuery(new SqlQuerySpec(builder.toString()), Bundle.class, Bundle.class));
  }
  ...
  private static void buildWhereConditions(
    ...
    if (StringUtils.isNotEmpty(idPsp)) {
      ...
      builder.append(
          "AND (IS_NULL(b.validityDateTo) OR SUBSTRING(DateTimeFromParts(b.validityDateTo[0],"
              + " b.validityDateTo[1], b.validityDateTo[2], 0, 0, 0, 0), 0, 10) >"
              + " SUBSTRING(GetCurrentDateTime(), 0, 10)) ");
    }
  ...
  /**
   * Build and execute the query to get all bundles filtered by name, type and validity date on
   * bundles table. validFrom param is used for exclude all bundle that are not active before the
   * specified date.
   *
  ...
  public List<Bundle> getBundlesByNameAndTypeAndValidityDateFromAndExpireAt(
    ...
    buildWhereForGetBundlesByNameAndTypeAndValidityDateFromAndExpireAt(
        name, types, validFrom, expireAt, active, builder);
  ...
  /**
   * Build and execute the query to get all bundles filtered by name, PSP business name and type on
   * bundles table.
   *
   * @param name bundle's name
  ...
  public List<Bundle> getBundlesByNameAndPSPBusinessName(
      String name, String pspBusinessName, String bundleType) {
    StringBuilder builder = new StringBuilder();
    builder.append(BASE_BUNDLES_QUERY);
  ...
  /**
   * Build and execute the query to count all bundles filtered by name, type and validity date on
   * bundles table. validFrom param is used for exclude all bundle that are not active before the
   * specified date.
   *
   * @param name bundle's name
  ...
  public Long getTotalItemsFindByNameAndTypeAndValidityDateFromAndExpireAt(
      String name, List<BundleType> types, LocalDate validFrom, LocalDate expireAt) {
    StringBuilder builder = new StringBuilder();
    builder.append("SELECT VALUE COUNT(1) FROM bundles b WHERE 1=1 ");

    buildWhereForGetBundlesByNameAndTypeAndValidityDateFromAndExpireAt(
        name, types, validFrom, expireAt, false, builder);
  ...
  private void buildWhereForGetBundlesByNameAndTypeAndValidityDateFromAndExpireAt(
    ...
    if (active != null) {
      ...
      baseString =
          "AND SUBSTRING(DateTimeFromParts(b.validityDateFrom[0], b.validityDateFrom[1],"
              + " b.validityDateFrom[2], 0, 0, 0, 0), 0, 10) ";
      builder
          .append(baseString)
          .append("<=")
          ...
          .append(", 0, 0, 0, 0), 0, 10) ");
      baseString =
          "AND SUBSTRING(DateTimeFromParts(b.validityDateTo[0], b.validityDateTo[1],"
              + " b.validityDateTo[2], 0, 0, 0, 0), 0, 10) ";
      builder
          .append(baseString)
          .append(">=")
          .append(" SUBSTRING(DateTimeFromParts(")
  ...
  private void buildWhereForGetBundlesByNameAndPSPBusinessName(
      String name, String pspBusinessName, String bundleType, StringBuilder builder) {
    if (StringUtils.isNotEmpty(name)) {
      builder.append(AND_BUNDLE_NAME_LIKE).append(name).append("%' ");
    }
  ...
  private static void buildDateQuery(
      LocalDate date, boolean isExpireDate, boolean isDateAfter, StringBuilder builder) {
    String baseString;
    if (isExpireDate) {
      baseString =
          "AND SUBSTRING(DateTimeFromParts(b.validityDateTo[0], b.validityDateTo[1],"
