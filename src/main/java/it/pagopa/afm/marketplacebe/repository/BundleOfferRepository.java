...
public interface BundleOfferRepository extends CosmosRepository<BundleOffer, String> {
  ...
  List<BundleOffer> findByIdPspAndIdBundleAndAcceptedDateIsNullAndRejectionDateIsNull(
      String idPsp, String idBundle);

  Optional<BundleOffer> findByIdBundleAndCiFiscalCodeAndAcceptedDateIsNullAndRejectionDateIsNull(
      String idBundle, String ciFiscalCode);

  List<BundleOffer> findByCiFiscalCode(String ciFiscalCode);

  @Query(
      value =
          "SELECT * FROM bundleoffers b WHERE (SUBSTRING(DateTimeFromParts(b.validityDateTo[0],"
