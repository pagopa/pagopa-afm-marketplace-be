...
public interface PaymentMethodRepository extends CosmosRepository<PaymentMethod, String> {
  ...
  @Query(
      "SELECT * FROM c WHERE ARRAY_CONTAINS(c.user_touchpoint, @touchpoint) AND"
          + " ARRAY_CONTAINS(c.user_device, @device)")
  List<PaymentMethod> findByTouchpointAndDevice(
      @Param("touchpoint") String touchpoint, @Param("device") String device);
