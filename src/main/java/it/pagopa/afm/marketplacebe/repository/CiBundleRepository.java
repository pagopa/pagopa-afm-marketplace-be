...
public interface CiBundleRepository extends CosmosRepository<CiBundle, String> {
  ...
  Integer getTotalItemsFindByIdBundleAndCiFiscalCode(
      @Param("idBundle") String idBundle, @Param("ciFiscalCode") String ciFiscalCode);

  Optional<CiBundle> findByIdBundleAndCiFiscalCode(String idBundle, String ciFiscalCode);

  Optional<CiBundle> findByIdBundleAndCiFiscalCodeAndValidityDateToIsNull(
      String idBundle, String ciFiscalCode);

  @Query(
      value =
          "SELECT * FROM cibundles b WHERE (SUBSTRING(DateTimeFromParts(b.validityDateTo[0],"
