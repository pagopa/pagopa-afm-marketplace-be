...
class CiControllerTest {
  ...
  void getBundlesByFiscalCode_200() throws Exception {
    when(bundleService.getBundlesByFiscalCode(
            anyString(),
            nullable(BundleType.class),
  ...
  void getBundleByFiscalCode_200() throws Exception {
    when(bundleService.getBundleByFiscalCode(anyString(), anyString()))
  ...
  void removeBundleByFiscalCode_404() throws Exception {
    AppException exception =
        new AppException(AppError.BUNDLE_NOT_FOUND, TestUtil.getMockIdBundle());
  ...
  void getBundleAttributesByFiscalCode_200() throws Exception {
    when(bundleService.getBundleAttributesByFiscalCode(anyString(), anyString()))
        .thenReturn(TestUtil.getMockBundleDetailsAttributes());
  ...
  void createBundleAttributesByCi_201() throws Exception {
    when(bundleService.createBundleAttributesByCi(anyString(), anyString(), any()))
        .thenReturn(TestUtil.getMockBundleAttributeResponse());
  ...
  void createBundleAttributesByCi_400() throws Exception {
    AppException exception =
        new AppException(AppError.BUNDLE_BAD_REQUEST, TestUtil.getMockIdBundle());
    doThrow(exception)
        .when(bundleService)
        .createBundleAttributesByCi(anyString(), anyString(), any());
  ...
  void updateBundleAttributesByCi_400() throws Exception {
    ...
    doThrow(exception)
        .when(bundleService)
        .updateBundleAttributesByCi(anyString(), anyString(), anyString(), any());
  ...
  void removeBundleAttributesByCi_400() throws Exception {
    ...
    doThrow(exception)
        .when(bundleService)
        .removeBundleAttributesByCi(anyString(), anyString(), anyString());
  ...
  void createRequest_201() throws Exception {
    when(bundleRequestService.createBundleRequest(anyString(), any()))
        .thenReturn(TestUtil.getMockBundleRequestId());
  ...
  void getOffersByCI_200() throws Exception {
    when(bundleOfferService.getCiOffers(anyString(), anyString(), any(), anyInt(), anyInt()))
        .thenReturn(TestUtil.getMockBundleCiOffers());
  ...
  void acceptOffer_201() throws Exception {
    when(bundleOfferService.acceptOffer(anyString(), anyString(), anyList()))
        .thenReturn(TestUtil.getMockCiBundleId());
            ...
            post(url)
                .content(
                    objectMapper.writeValueAsString(
                        Collections.singletonList(new CiBundleAttributeModel())))
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isCreated());
  }
  ...
  void acceptOffer_409() throws Exception {
    AppException exception =
        new AppException(AppError.BUNDLE_OFFER_ALREADY_ACCEPTED, TestUtil.getMockIdBundle(), "");
    doThrow(exception).when(bundleOfferService).acceptOffer(anyString(), anyString(), anyList());
  ...
  void rejectOffer_404() throws Exception {
    ...
    mvc.perform(post(url).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());
  }

  @Test
  void rejectOffer_409() throws Exception {
    AppException exception =
        new AppException(AppError.BUNDLE_OFFER_ALREADY_REJECTED, TestUtil.getMockIdBundle(), "");
    doThrow(exception).when(bundleOfferService).rejectOffer(anyString(), anyString());

    String url = String.format(REJECT_OFFER, TestUtil.getMockCiFiscalCode(), "idBundleOffer");

    mvc.perform(post(url).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isConflict());
  }
}
