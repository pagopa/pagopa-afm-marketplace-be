...
class PspControllerTest {
  ...
  @MockBean private BundleOfferService bundleOfferService;
  @MockBean private BundleRequestService bundleRequestService;

  @Test
  void getBundles_200() throws Exception {
    when(bundleService.getBundlesByIdPsp(
            anyString(),
            anyList(),
            any(),
            any(),
            ...
            anyInt(),
            anyInt()))
        .thenReturn(TestUtil.getMockBundles());
  ...
  void getBundle_200() throws Exception {
    when(bundleService.getBundleById(anyString(), anyString()))
        .thenReturn(TestUtil.getMockPspBundleDetails());
  ...
  void getBundleCreditorInstitutions_200() throws Exception {
    when(bundleService.getCIs(anyString(), anyString(), nullable(String.class), anyInt(), anyInt()))
        .thenReturn(TestUtil.getMockBundleCreditorInstitutionResource());
  ...
  void getBundleCreditorInstitutions_404() throws Exception {
    ...
    doThrow(exception)
        .when(bundleService)
        .getCIs(anyString(), anyString(), nullable(String.class), anyInt(), anyInt());
  ...
  void getBundleCreditorInstitutionDetails_200() throws Exception {
    when(bundleService.getCIDetails(anyString(), anyString(), anyString()))
        .thenReturn(TestUtil.getMockCiBundleDetails());

    String url =
        String.format(
            CI_BUNDLES + "/%s",
            TestUtil.getMockIdPsp(),
            TestUtil.getMockIdBundle(),
            TestUtil.getMockCiFiscalCode());
  ...
  void createBundle_201() throws Exception {
    when(bundleService.createBundle(anyString(), any()))
        .thenReturn(TestUtil.getMockBundleResponse());
  ...
  void createBundleByList_201() throws Exception {
    when(bundleService.createBundleByList(anyString(), any()))
        .thenReturn(TestUtil.getMockBundleResponseList());
  ...
  void updateBundle_200() throws Exception {
    when(bundleService.updateBundle(anyString(), anyString(), any(), anyBoolean()))
        .thenReturn(TestUtil.getMockBundle());
  ...
  void updateBundle_400() throws Exception {
    ...
    doThrow(exception)
        .when(bundleService)
        .updateBundle(anyString(), anyString(), any(), anyBoolean());
  ...
  void sendBundleOffer_201() throws Exception {
    when(bundleOfferService.sendBundleOffer(anyString(), anyString(), any()))
        .thenReturn(List.of(TestUtil.getMockBundleOffered()));
  ...
  void removeBundleOffer_400() throws Exception {
    AppException exception =
        new AppException(AppError.BUNDLE_OFFER_BAD_REQUEST, "idbundle", "name");
    doThrow(exception)
        .when(bundleOfferService)
        .removeBundleOffer(anyString(), anyString(), anyString());
  ...
  void getRequestsByPsp_200() throws Exception {
    when(bundleRequestService.getPublicBundleRequests(
            anyString(), anyInt(), anyInt(), anyString(), anyString()))
        .thenReturn(TestUtil.getMockPublicBundleRequests());
  ...
  void acceptRequest_201() throws Exception {
    ...
    mvc.perform(post(url).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isCreated());
  }

  @Test
  void acceptRequest_404() throws Exception {
    AppException exception =
        new AppException(AppError.BUNDLE_REQUEST_NOT_FOUND, "idbundle", "name");
  ...
  void acceptRequest_409() throws Exception {
    AppException exception =
        new AppException(AppError.REQUEST_ALREADY_ACCEPTED, "idbundle", "name");
    doThrow(exception).when(bundleRequestService).acceptRequest(anyString(), anyString());
  ...
  void rejectRequest_200() throws Exception {
    ...
    mvc.perform(post(url).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
  }
  ...
  void rejectRequest_409() throws Exception {
    AppException exception =
        new AppException(AppError.REQUEST_ALREADY_REJECTED, "idbundle", "name");
    doThrow(exception).when(bundleRequestService).rejectRequest(anyString(), anyString());
