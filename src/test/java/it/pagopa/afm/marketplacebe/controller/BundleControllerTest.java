...
class BundleControllerTest {
  ...
  void getGlobalBundles() throws Exception {
    when(bundleService.getBundles(anyList(), anyString(), any(), any(), any(), anyInt(), anyInt()))
  ...
  void getBundleDetails() throws Exception {
    when(bundleService.getBundleDetailsById(anyString()))
        .thenReturn(TestUtil.getMockPspBundleDetails());

    String url = "/bundles/{id-bundle}";
    mvc.perform(get(url, "bundleId").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON));
  }
}
