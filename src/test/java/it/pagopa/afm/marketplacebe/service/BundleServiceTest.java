...
class BundleServiceTest {
  @Captor ArgumentCaptor<Bundle> bundleArgumentCaptor = ArgumentCaptor.forClass(Bundle.class);
  @Captor ArgumentCaptor<CiBundle> ciBundleArgumentCaptor = ArgumentCaptor.forClass(CiBundle.class);
  @MockBean private BundleRepository bundleRepository;
  @MockBean private CiBundleRepository ciBundleRepository;
  @MockBean private BundleRequestRepository bundleRequestRepository;
  @MockBean private TouchpointRepository touchpointRepository;
  @MockBean private ValidBundlesTaskExecutor validBundlesTaskExecutor;
  @MockBean private BundleOfferRepository bundleOfferRepository;
  @MockBean private CosmosRepository cosmosRepository;
  @MockBean private PaymentTypeRepository paymentTypeRepository;
  @MockBean private ArchivedBundleRepository archivedBundleRepository;
  @MockBean private ArchivedBundleOfferRepository archivedBundleOfferRepository;
  @MockBean private ArchivedBundleRequestRepository archivedBundleRequestRepository;
  @MockBean private ArchivedCiBundleRepository archivedCiBundleRepository;
  @MockBean private ValidBundleRepository validBundleRepository;

  @MockBean private TaskManager taskManager;
  ...
  void shouldGetBundles(int bundleTypeSize) {
    ...
    // Precondition
    when(cosmosRepository.getBundlesByNameAndTypeAndValidityDateFromAndExpireAt(
            eq(null), anyList(), eq(null), eq(null), eq(null), anyInt(), anyInt()))
        .thenReturn(bundleList);
    when(cosmosRepository.getTotalItemsFindByNameAndTypeAndValidityDateFromAndExpireAt(
            eq(null), anyList(), eq(null), eq(null)))
    ...
    Bundles bundles =
        assertDoesNotThrow(() -> bundleService.getBundles(list, null, null, null, null, 50, 0));
  ...
  void shouldGetBundlesByName() {
    ...
    // Precondition
    when(cosmosRepository.getBundlesByNameAndTypeAndValidityDateFromAndExpireAt(
            anyString(), anyList(), eq(null), eq(null), eq(null), anyInt(), anyInt()))
        .thenReturn(bundleList);
    when(cosmosRepository.getTotalItemsFindByNameAndTypeAndValidityDateFromAndExpireAt(
            anyString(), anyList(), eq(null), eq(null)))
        .thenReturn((long) bundleList.size());
  ...
  void shouldGetBundlesByValidDate() {
    ...
    // Precondition
    when(cosmosRepository.getBundlesByNameAndTypeAndValidityDateFromAndExpireAt(
            eq(null), anyList(), any(), any(), any(), anyInt(), anyInt()))
        .thenReturn(bundleList);
    when(cosmosRepository.getTotalItemsFindByNameAndTypeAndValidityDateFromAndExpireAt(
            eq(null), anyList(), any(), any()))
        .thenReturn((long) bundleList.size());
    ...
    Bundles bundles =
        bundleService.getBundles(
            bundleTypeList, null, LocalDate.now(), LocalDate.now(), null, 50, 0);
  ...
  void shouldGetBundlesByIdPsp() {
    ...
    // Precondition
    when(cosmosRepository.getBundlesByNameAndType(
            anyString(),
            any(),
            anyList(),
            any(),
    ...
    Bundles bundles =
        bundleService.getBundlesByIdPsp(
            bundle.getIdPsp(),
            bundleParams,
            null,
            Sort.Direction.ASC,
  ...
  void shouldGetBundleById() {
    ...
    PspBundleDetails requestedBundles =
  ...
  void shouldRaiseNotFoundException() {
    ...
    AppException exc =
        assertThrows(AppException.class, () -> bundleService.getBundleById(idBundle, idPsp));
  ...
  void shouldCreateBundle() {
    ...
    when(bundleRepository.save(Mockito.any())).thenReturn(bundle);
    ...
    assertEquals(
        bundleRequest.getPspBusinessName(), bundleArgumentCaptor.getValue().getPspBusinessName());
  }
  ...
  void shouldCreateBundleWithPaymentTypeNotFound() {
    ...
    AppException exception =
        assertThrows(AppException.class, () -> bundleService.createBundle(idPsp, bundleRequest));
  ...
  void shouldRaiseBadRequestWithOverlappingBundle() {
    ...
    when(paymentTypeRepository.findByName(anyString()))
        .thenReturn(Optional.of(TestUtil.getMockPaymentType()));
    when(bundleRepository.findByIdPspAndTypeAndPaymentTypeAndTouchpoint(
  ...
  void shouldRaiseBadRequestWithWrongAmountRangeCreate() {
    ...
    assertTrue(
        appException
            .getMessage()
            .contains("Amount range not valid. MaxPaymentAmount must be >= than MinPaymentAmount"));
  }
  ...
  void shouldDeleteBundle() {
    ...
    when(bundleRequestRepository.findByIdBundleAndIdPspAndAcceptedDateIsNullAndRejectionDateIsNull(
            anyString(), anyString()))
        .thenReturn(requests);
    when(bundleOfferRepository.findByIdPspAndIdBundleAndAcceptedDateIsNullAndRejectionDateIsNull(
            anyString(), anyString()))
        .thenReturn(offers);
  ...
  void removeBundle_ko_1() {
    ...
    String idPsp = bundle.getIdPsp();
    AppException e =
        assertThrows(AppException.class, () -> bundleService.removeBundle(bundleId, idPsp));
  ...
  void shouldGetCIs() {
    ...
    BundleCreditorInstitutionResource ciFiscalCodeList =
    ...
    assertEquals(
        ciBundles.get(0).getCiFiscalCode(),
        ciFiscalCodeList.getCiBundleDetails().get(0).getCiTaxCode());
  }
  ...
  void shouldGetCIDetails() {
    ...
    CiBundleDetails ciBundleDetails =
        bundleService.getCIDetails(bundle.getId(), bundle.getIdPsp(), ciBundle.getCiFiscalCode());

    assertEquals(ciBundle.getAttributes().size(), ciBundleDetails.getAttributes().size());
    assertEquals(
        ciBundle.getAttributes().get(0).getMaxPaymentAmount(),
        ...
        assertThrows(
            ...
            () ->
                bundleService.getCIDetails(
                    bundle.getId(), bundle.getIdPsp(), ciBundle.getCiFiscalCode()));
  ...
  void shouldGetBundlesByFiscalCode() {
    ...
    // Preconditions
    when(ciBundleRepository.findByCiFiscalCodeAndTypeAndIdBundles(
            anyString(), eq(null), eq(null), anyInt(), anyInt()))
        .thenReturn(ciBundles);
    when(ciBundleRepository.getTotalItemsFindByCiFiscalCodeAndTypeAndIdBundles(
            anyString(), eq(null), eq(null)))
        .thenReturn(1);

    CiBundles ciBundlesResult =
        bundleService.getBundlesByFiscalCode(
            mockCIBundle.getCiFiscalCode(), null, null, null, 100, 0);

    assertEquals(ciBundles.size(), ciBundlesResult.getBundleDetailsList().size());
    assertEquals(
        mockCIBundle.getIdBundle(), ciBundlesResult.getBundleDetailsList().get(0).getIdBundle());
  ...
  void shouldGetBundlesByFiscalCodeTypeFilter() {
    ...
    // Preconditions
    when(ciBundleRepository.findByCiFiscalCodeAndTypeAndIdBundles(
            mockCIBundle.getCiFiscalCode(), "GLOBAL", null, 0, 100))
        .thenReturn(new ArrayList<>());
    when(ciBundleRepository.getTotalItemsFindByCiFiscalCodeAndTypeAndIdBundles(
            mockCIBundle.getCiFiscalCode(), "GLOBAL", null))
        .thenReturn(0);
  ...
  void shouldGetBundlesByFiscalCodeAndPspBusinessNameFilter() {
    ...
    // Preconditions
    when(ciBundleRepository.findByCiFiscalCodeAndTypeAndIdBundles(
            anyString(), eq(null), anyList(), anyInt(), anyInt()))
        .thenReturn(ciBundles);
    when(ciBundleRepository.getTotalItemsFindByCiFiscalCodeAndTypeAndIdBundles(
            anyString(), eq(null), anyList()))
        .thenReturn(1);
    when(cosmosRepository.getBundlesByNameAndPSPBusinessName(
            bundle.getPspBusinessName(), null, null))
        .thenReturn(List.of(bundle));

    CiBundles ciBundlesResult =
        bundleService.getBundlesByFiscalCode(
            mockCIBundle.getCiFiscalCode(), null, null, bundle.getPspBusinessName(), 100, 0);

    assertEquals(ciBundles.size(), ciBundlesResult.getBundleDetailsList().size());
    assertEquals(
        mockCIBundle.getIdBundle(), ciBundlesResult.getBundleDetailsList().get(0).getIdBundle());
    assertEquals(
        mockCIBundle.getId(), ciBundlesResult.getBundleDetailsList().get(0).getIdCIBundle());
    assertEquals(0, ciBundlesResult.getPageInfo().getPage());
    assertEquals(1, ciBundlesResult.getPageInfo().getTotalItems());
    assertEquals(1, ciBundlesResult.getPageInfo().getTotalPages());
  }
  ...
  void shouldGetBundlesByFiscalCodeAndTypeAndPspBusinessNameFilters() {
    ...
    when(ciBundleRepository.getTotalItemsFindByCiFiscalCodeAndTypeAndIdBundles(
        ...
        .thenReturn(1);
    when(cosmosRepository.getBundlesByNameAndPSPBusinessName(
            bundle.getPspBusinessName(), null, "PRIVATE"))
        .thenReturn(List.of(bundle));
  ...
  void shouldGetBundlesByFiscalCodeAndTypeAndBundleNameFilters() {
    ...
    CiBundles ciBundlesResult =
        bundleService.getBundlesByFiscalCode(
            mockCIBundle.getCiFiscalCode(), BundleType.PRIVATE, bundle.getName(), null, 100, 0);
  ...
  void shouldGetBundleByFiscalCode() {
    ...
    CiBundleDetails bundleDetails =
        bundleService.getBundleByFiscalCode(ciBundle.getCiFiscalCode(), bundle.getId());
  ...
  void shouldRemoveByFiscalCode() {
    ...
    bundleService.removeBundleByFiscalCode(ciBundle.getCiFiscalCode(), ciBundle.getIdBundle());
  ...
  void shouldGetBundleAttributeByFiscalCode() {
    ...
    BundleDetailsAttributes bundleDetailsAttributes =
        bundleService.getBundleAttributesByFiscalCode(
            ciBundle.getCiFiscalCode(), ciBundle.getIdBundle());

    assertEquals(
        ciBundle.getAttributes().get(0).getId(),
        bundleDetailsAttributes.getAttributes().get(0).getId());
  }
  ...
  void shouldCreateBundleAttributesByCi_1() {
    ...
    bundleService.createBundleAttributesByCi(
        ciBundle.getCiFiscalCode(), bundle.getId(), getMockBundleAttribute());
  ...
  void shouldRaiseBadRequestCreateBundleAttributesByCi_1() {
    ...
    AppException appException =
        assertThrows(
            AppException.class,
            () -> bundleService.createBundleAttributesByCi(fiscalCode, idBundle, attributes));
  ...
  void shouldRaiseBadRequestUpdateBundleAttributesByCi_1() {
    ...
    AppException appException =
        assertThrows(
            ...
            () ->
                bundleService.updateBundleAttributesByCi(
                    fiscalCode, idBundle, idAttribute, attributes));
  ...
  void createBundle_ok_1() {
    when(bundleRepository.findByIdPspAndTypeAndPaymentTypeAndTouchpoint(
            anyString(), any(BundleType.class), anyString(), anyString()))
    ...
    BundleResponse result =
        bundleService.createBundle(TestUtil.getMockIdPsp(), TestUtil.getMockBundleRequest());
    assertNotNull(result);
  }

  @Test
  void createBundle_ok_2() {
    // same (payment method, touchpoint, type), different payment amount range and
  ...
  void createBundle_ok_3() {
    // same (payment method, touchpoint, type, transferCategoryList, payment amount range),
    // different validityDate
    BundleRequest bundleRequest = TestUtil.getMockBundleRequest();
  ...
  void createBundle_ok_5() {
    ...
    when(touchpointRepository.findByName(bundleRequest.getTouchpoint()))
        .thenReturn(Optional.of(TestUtil.getMockTouchpoint(bundleRequest.getTouchpoint())));
    when(paymentTypeRepository.findByName(bundleRequest.getPaymentType()))
        .thenReturn(Optional.of(TestUtil.getMockPaymentType()));
  ...
  void createBundle_ok_8() {
    ...
    BundleResponse result1 =
        bundleService.createBundle(TestUtil.getMockIdPsp(), TestUtil.getMockBundleRequest());
    assertNotNull(result1);
    BundleResponse result2 =
        bundleService.createBundle(TestUtil.getMockIdPsp(), TestUtil.getMockBundleRequest());
  ...
  void createBundle_ok_11() {
    // same (payment method, touchpoint, transferCategoryList, payment amount range), idChannel ONUS
    // case
    BundleRequest bundleRequest = TestUtil.getMockBundleRequest();
  ...
  void createBundle_ok_12() {
    // same (payment method, touchpoint, transferCategoryList, payment amount range), idChannel ONUS
    // case and transferCategoryList null
    BundleRequest bundleRequest = TestUtil.getMockBundleRequest();
    bundleRequest.setOnUs(true);
  ...
  void createBundle_ok_13() {
    // same (payment method, touchpoint, transferCategoryList, payment amount range, idChannel), and
    // bundle type is PRIVATE.
    BundleRequest bundleRequest = TestUtil.getMockBundleRequest();
    bundleRequest.setType(BundleType.PRIVATE);
  ...
  void createBundle_ko_11() {
    // Same bundle configuration for two bundle (1 ONUS, 1 NOT ONUS) with different normalized
    // idChannel.
    BundleRequest bundleRequest = TestUtil.getMockBundleRequest();
    List<Bundle> bundles = TestUtil.getMockBundleSameConfiguration();
  ...
  void updateBundle_ok_1() {
    ...
    when(ciBundleRepository.findByIdBundle(anyString()))
        .thenReturn(List.of(TestUtil.getMockCiBundle()));
    when(paymentTypeRepository.findByName(bundle.getPaymentType()))
        .thenReturn(Optional.of(TestUtil.getMockPaymentType()));

    Bundle result =
        bundleService.updateBundle(
            TestUtil.getMockIdPsp(), bundle.getId(), TestUtil.getMockBundleRequest(), false);
    assertNotNull(result);
  ...
  void updateBundle_ok_3() {
    ...
    Bundle result =
        bundleService.updateBundle(TestUtil.getMockIdPsp(), bundle.getId(), bundleRequest, false);
    assertNotNull(result);
  }
  ...
  void createBundleByList_ok() {
    ...
    List<BundleResponse> result =
        bundleService.createBundleByList(
            TestUtil.getMockIdPsp(), TestUtil.getMockBundleRequestList());
    assertNotNull(result);
    ...
    assertEquals(
        iter.next().getPspBusinessName(),
        TestUtil.getMockBundleRequestList().get(0).getPspBusinessName());
    assertEquals(
        iter.next().getPspBusinessName(),
        TestUtil.getMockBundleRequestList().get(1).getPspBusinessName());
    assertEquals(
        iter.next().getPspBusinessName(),
        TestUtil.getMockBundleRequestList().get(2).getPspBusinessName());
  }
  ...
  void getBundleByIdSuccess() {
    ...
    PspBundleDetails result =
        assertDoesNotThrow(() -> bundleService.getBundleDetailsById("bundleId"));
  ...
  void getBundleByIdNotFound() {
    ...
    AppException e =
        assertThrows(AppException.class, () -> bundleService.getBundleDetailsById("bundleId"));
  ...
  void createBundle_ok_postePublicDifferentChannel() {
    // Poste PSP, PUBLIC bundle, same configuration as an existing one but different channel ->
    // allowed
    BundleRequest bundleRequest = TestUtil.getMockBundleRequest();
    bundleRequest.setType(BundleType.PUBLIC);
  ...
  void createBundle_ko_postePublicSameChannel() {
    ...
    AppException e =
        assertThrows(
            AppException.class, () -> bundleService.createBundle(POSTE_ID_PSP, bundleRequest));
  ...
  void updateBundle_ok_postePublicDifferentChannel() {
    ...
    when(bundleRepository.findById(anyString(), any(PartitionKey.class)))
        .thenReturn(Optional.of(bundleToUpdate));
    when(bundleRepository.findByIdPspAndTypeAndPaymentTypeAndTouchpoint(
            anyString(), any(BundleType.class), anyString(), anyString()))
        .thenReturn(List.of(otherBundle));
    ...
    Bundle result =
        bundleService.updateBundle(POSTE_ID_PSP, bundleToUpdate.getId(), bundleRequest, false);
